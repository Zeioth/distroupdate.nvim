...
* **Do this plugin send events?** The commands `:NvimUpdatePlugins` and `:NvimUpdateConfig` trigger the events `User MasonUpdateCompleted` and `User DistroUpdateCompleted`. Knowing this you can create a autocmd to listen these events in case you want something to happen after the commands finnish.
